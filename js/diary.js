  ...
  $.ajax({
    ...
    success: function(data){
      ...
      for( let i = 0; i < data.length; i++) {
        ...
        if ((today.getDate() == date.getDate()) && (today.getMonth() == date.getMonth()) && (today.getFullYear() != date.getFullYear()) && (data[i].docType == "journal")) {
          $('#todo-list').append(`<li><span>${date.getFullYear()} ${data[i].capsule}</span></li>`)
        }
      }
    },
...
var capsule = document.querySelector("input[name=capsule]");
